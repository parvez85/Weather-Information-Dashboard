...
function getWeather(q) {
    ...
    $.ajax({
        ...
        var cityMain1 = $("<div col-12>").append($("<p><h2>" + response.name + ' (' + currentDate + ')' + "</h2>"));
        var image = $('<img class="imgsize"><p>').attr('src', 'http://openweathermap.org/img/w/' + response.weather[0].icon + '.png');        
    ...
    $.ajax({ // gets the 5 day forecast API
        ...
        for (var i = 0; i < arrayList.length; i++) {
            if (arrayList[i].dt_txt.split(' ')[1] === '12:00:00') {
                ...
                var cityMain = $('<div>');
                cityMain.addClass('col forecast text-white ml-3 mb-3>');
